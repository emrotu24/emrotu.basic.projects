...
$pagina = 'acquisizione_anagrafiche_collegate';
...
function elaboraDati($dati, $riga, $id_acquisizione)
{
    ...
    global $tipi_collegati_anagrafica;
    global $tipi_collegati_pratica;
    ...
    // $ind_attivo = 0;
    // $ind_predefinito = 0;
    //$ind_corrispondenza = 0;
    //$ind_corrispondenza_predefinito = 0;


  /*  $ind_predefinito = (isset($_POST['ind_predefinito']) && $_POST['ind_predefinito'] != '') ? '1' : '0';
    $ind_corrispondenza = (isset($_POST['ind_corrispondenza']) && $_POST['ind_corrispondenza'] != '') ? '1' : '0';
    $ind_corrispondenza_predefinito = (isset($_POST['ind_corrispondenza_predefinito']) && $_POST['ind_corrispondenza_predefinito'] != '') ? '1' : '0';
    $ind_attivo = (isset($_POST['ind_primario_attivo']) && $_POST['ind_primario_attivo'] != '') ? '1' : '0';
...
*/
    ...
    $occorrenze = array_keys($campi, 'altro*-*tipologia_collegato_pratica');
    if (count($occorrenze) > 0) {
        ...
        for ($k = 0; $k < count($occorrenze); $k++) {
            ...
            try{
                $query_seleziona_tipo_garante = 'SELECT id_remida 
                                                        ...
                                                        AND (codice_uno = ?
                                                        OR codice_due = ?)';
                $selezione_tipo_garante = $mysqli->prepare($query_seleziona_tipo_garante);            
                $selezione_tipo_garante->bind_param("iss", $id_mandante, $risultante, $risultante);          
                
                $selezione_tipo_garante->execute();
                $ris_selezione_tipo_garante = $selezione_tipo_garante->get_result();
                $tipo_collegato = $ris_selezione_tipo_garante->fetch_assoc();

                $dett_univoco[$k]['tipologia_collegato_pratica'] = $tipo_collegato['id_remida'];

                $id_tipologia_collegato_pratica = $tipo_collegato['id_remida'];
                                    
                $mysqli->commit();  

            }catch (mysqli_sql_exception $e){
                echo 'ERRORE SELEZIONE TIPO GARANTE DA DECODIFICHE DETTAGLI: ' . $e->getMessage() . '<br>';
                $mysqli->rollback();
            };
            $mysqli->autocommit(TRUE);
            $selezione_tipo_garante->close();
        }
    }
    else {
        $dett_univoco[$k]['tipologia_collegato_pratica'] = $tipi_collegati_pratica[0];

        $id_tipologia_collegato_pratica = $tipi_collegati_pratica[0];
    }

    $tipo_anagrafica_collegato_anagrafica = false;
    $occorrenze = array_keys($campi, 'altro*-*tipologia_collegato_anagrafica');
    if (count($occorrenze) > 0) {
        $tipo_anagrafica_collegato_anagrafica = true;
        for ($k = 0; $k < count($occorrenze); $k++) {
            $risultante = calcola_risultante($dati, $occorrenze[$k], $_POST['funzione_pers'], $_POST['colonne_pers']);
			...
			// 										AND D.tipo = "tipo collegato anagrafica"
			// 										AND (codice_uno = "' . $risultante . '"
			// 										OR codice_due = "' . $risultante . '")';
            // $tipo_collegato = mysql_fetch_array(db_query($query_seleziona_tipo_garante));

            // $dett_univoco[$k]['tipologia_collegato_anagrafica'] = $tipo_collegato['id_remida'];

            // $id_tipologia_collegato_anagrafica = $tipo_collegato['id_remida'];
            ...
            try{
                ...
                echo 'ERRORE SELEZIONE TIPO COLLEGATO ANAGRAFICA DA DECODIFICHE DETTAGLI: ' . $e->getMessage() . '<br>';
                $mysqli->rollback();
            };
            $mysqli->autocommit(TRUE);
            $selezione_tipo_garante->close();
        ...
        $dett_univoco[$k]['tipologia_collegato_anagrafica'] = $tipi_collegati_anagrafica[0];

        $id_tipologia_collegato_anagrafica = $tipi_collegati_anagrafica[0];
    }
    ...
    if (count($occorrenze) > 0) {
        ...
        for ($k = 0; $k < count($occorrenze); $k++) {
            ...
            // $idDebTemp = db_fetch_array_assoc(db_query($qryIdDeb));
            ...
            $array_prt=[];
            foreach ($idDebTemp as $valTemp){
                $array_prt[]=$valTemp['id_pratica'];
            }

            if(count($array_prt)>0) {
                $dett_univoco[$k]['array_prt'] = $array_prt;
            }
        }
    ...
    $occorrenze = array_keys($campi, 'altro*-*codice_anagrafico_mandante');
    if (count($occorrenze) > 0) {
        $presence_codice_anagrafico_mandante = true;
        for ($k = 0; $k < count($occorrenze); $k++) {
            $risultante = calcola_risultante($dati, $occorrenze[$k], $_POST['funzione_pers'], $_POST['colonne_pers']);

            // $query = 'SELECT id_collegato_pratica FROM anagrafica_collegati_mandante WHERE codice_anagrafico_mandante = "' . $risultante . '" AND id_mandante = "' . db_input($id_mandante) . '"';
            // $row_codice_mandante = mysql_fetch_array(db_query($query));
    ...
    if ($evento_strutturato_predefinito_indirizzi > 0 && isset($id_utente) && $id_utente > 0) {
    ...
    } else if (isset($id_pratica) && $id_pratica > 0)
        $eventi_strutturati[$risultante] .= $id_pratica . ',';

    if ($evento_strutturato_predefinito_recapiti > 0 && isset($id_utente) && $id_utente > 0) {
    ...
    if (count($occorrenze) > 0 && isset($id_utente) && $id_utente > 0) {
        $stringa_pratiche = '';
    ...
    if (trim($dett_univoco[0]['id_utente']) != '' || trim($dett_univoco[0]['id_pratica']) != '') {

        ?>

        <br><br>INSERIMENTO UTENTE:<br>

        <?php
        {
            $query_utente = array();
            $query_utente_acm = array();
...

            {
                ...
                /*
					...
					anagrafica*-*note
					*/
                $occorrenze = array_keys($campi, 'anagrafica*-*nome');
                for ($k = 0; $k < count($occorrenze); $k++) {
                    $risultante = calcola_risultante($dati, $occorrenze[$k], $_POST['funzione_pers'], $_POST['colonne_pers']);
                    ...
                    if ($risultante != '') {
                        ...
                        $query_utente[0] .= 'nome = "' . $risultante . '", ';
                        $dett_utente[0]['nome'] = $risultante;
                    }
                }
                $occorrenze = array_keys($campi, 'anagrafica*-*cognome');
                for ($k = 0; $k < count($occorrenze); $k++) {
                    $risultante = calcola_risultante($dati, $occorrenze[$k], $_POST['funzione_pers'], $_POST['colonne_pers']);
                    ...
                    if ($risultante != '') {
                        ...
                        //$query_utente[$k] .= 'codice_fiscale = "'.$risultante.'", ';
                        //$dett_utente[$k]['codice_fiscale'] = $risultante;
                        $query_utente[0] .= 'cognome = "' . $risultante . '", ';
                        $dett_utente[0]['cognome'] = $risultante;
                    }
                }

                $occorrenze = array_keys($campi, 'anagrafica*-*codice_anagrafico_mandante');
                for ($k = 0; $k < count($occorrenze); $k++) {
                    $risultante = calcola_risultante($dati, $occorrenze[$k], $_POST['funzione_pers'], $_POST['colonne_pers']);

                    if ($risultante != '') {
                        $query_utente_acm[0] .= 'codice_anagrafico_mandante = "' . $risultante . '", ';
                        $dett_utente_acm[0]['codice_anagrafico_mandante'] = $risultante;
                    }
                }
                ...
                if ($esistenza_recapito) {
                    ...
                    if ($ind_corrispondenza_predefinito > 0) {
                        $query_recapito[0] .= 'invio_corrispondenza = "' . db_input($ind_corrispondenza_predefinito) . '", ';
                        $dett_recapito[0]['invio_corrispondenza'] = $ind_corrispondenza_predefinito;
                    } else {
                    ...
                    if ($ind_predefinito > 0) {
                        $query_recapito[0] .= 'predefinito = "' . db_input($ind_predefinito) . '", ';
                        $dett_recapito[0]['predefinito'] = $ind_predefinito;
                    ...
                    if ($ind_attivo > 0) {
                        $query_recapito[0] .= 'attivo = "' . db_input($ind_attivo) . '", ';
                        $dett_recapito[0]['attivo'] = $ind_attivo;
                    } else {
                    ...
                    $occorrenze = array_keys($campi, 'anagrafica*-*nazione');
                    if (count($occorrenze) > 0) {
                        for ($k = 0; $k < count($occorrenze); $k++) {
                            $risultante = calcola_risultante($dati, $occorrenze[$k], $_POST['funzione_pers'], $_POST['colonne_pers']);
                ...
                for ($k = 0; $k < count($occorrenze); $k++) {
                    ...
                    if ($rec_tipo <= 0 && count($occorrenze_tipo_recapito) <= 0 && $tipi_recapito_telefonico[$occorrenze[$k]] > 0) {
                ...
                if ($esistenza_recapito) {
                    ...
                    for ($k = 0; $k < count($occorrenze); $k++) {
                        if ($rec_tipo > 0) {
                            ...
                            $query_recapito[$posizioni_indirizzi[$occorrenze[$k]]] .= 'tipo_recapito = "' . db_input($rec_tipo) . '", ';
                            $dett_recapito[$posizioni_indirizzi[$occorrenze[$k]]]['tipo_recapito'] = $rec_tipo;
                        } else {
                            $risultante = calcola_risultante($dati, $occorrenze[$k], $_POST['funzione_pers'], $_POST['colonne_pers']);
                            ...
                            if ($risultante != '') {
                                ...
                                $query_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]] .= 'tipo_recapito_telefonico = "' . db_input($tipo_indirizzo['id_remida']) . '", ';
                                $dett_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]]['tipo_recapito_telefonico'] = $tipo_indirizzo['id_remida'];
                            }
                        }
                    }

                    if ($rec_corrispondenza > 0) {
                        $query_recapito_telefonico[0] .= 'usa_per_invio = "' . db_input($rec_corrispondenza) . '", ';
                        $dett_recapito_telefonico[0]['usa_per_invio'] = $rec_corrispondenza;
                    } else {
                        ...
                        for ($k = 0; $k < count($occorrenze); $k++) {
                            ...
                            if ($risultante != '') {
                                $query_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]] .= 'usa_per_invio = "' . $risultante . '", ';
                                $dett_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]]['usa_per_invio'] = $risultante;
                            }
                        }
                    }

                    if ($rec_predefinito > 0) {
                        $query_recapito_telefonico[0] .= 'principale = "' . db_input($rec_predefinito) . '", ';
                        $dett_recapito_telefonico[0]['principale'] = $rec_predefinito;
                        ...
                        for ($k = 0; $k < count($occorrenze); $k++) {
                            ...
                            if ($risultante != '') {
                                $query_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]] .= 'principale = "' . $risultante . '", ';
                                $dett_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]]['principale'] = $risultante;
                    ...
                    if ($rec_attivo > 0) {
                        $query_recapito_telefonico[0] .= 'attivo = "' . db_input($rec_attivo) . '", ';
                        $dett_recapito_telefonico[0]['attivo'] = $rec_attivo;
                    } else {
                        ...
                        for ($k = 0; $k < count($occorrenze); $k++) {
                            ...
                            if ($risultante != '') {
                                $query_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]] .= 'attivo = "' . $risultante . '", ';
                                $dett_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]]['attivo'] = $risultante;
                            }
                        }
                    }
                    ...
                    for ($k = 0; $k < count($occorrenze); $k++) {
                        ...
                        if ($risultante != '') {
                            ...
                            $query_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]] .= 'note = "' . db_input($tipo_indirizzo['id_remida']) . '", ';
                            $dett_recapito_telefonico[$posizioni_indirizzi[$occorrenze[$k]]]['note'] = $tipo_indirizzo['id_remida'];
                        }
                    }
                }
            }
        ...
        // INSERIMENTO UTENTE // FUNZIONA
        for ($v = 0; $v < count($query_utente); $v++) {
            $esistenza_utente = false;


            $gruppo_base = '';
            if (isset($id_utente) && $id_utente > 0) {
                $gruppo_base = 'gruppi_base = 10, ';
            } else if (isset($id_pratica) && $id_pratica > 0) {
                $gruppo_base = 'gruppi_base = 8, ';
            }

            $cf = db_input(str_replace(' ', '', $dett_utente[0]['codice_fiscale']));
            $pi = db_input(str_replace(' ', '', $dett_utente[0]['partita_iva']));
            ...
            //echo $query_esistenza_utente;
            //echo '<br>';

            if($query_esistenza_utente != '') {
                // $ris_esistenza_utente = db_query($query_esistenza_utente);

                $mysqli->autocommit(FALSE);
                try{
                    $select_esistenza_utente = $mysqli->prepare($query_esistenza_utente);
                    $select_esistenza_utente->execute();
                    $ris_esistenza_utente = $select_esistenza_utente->get_result();
                    // $ris_esistenza_utente = $result->fetch_assoc();
                                        
                    $mysqli->commit();  

                }catch (mysqli_sql_exception $e){
                    echo 'ERRORE RISULTATO SELEZIONE ESISTENZA UTENTE: ' . $e->getMessage() . '<br>';
                    $mysqli->rollback();
                };
                $mysqli->autocommit(TRUE);
                $select_esistenza_utente->close();

                if ($ris_esistenza_utente->num_rows > 0) {
                    $esistenza_utente = true;
                    //echo 'UTENTE TROVATO';
                }
                else {
            ...
            // TO-DO ------ CONTINUARE SOTTO ----------
            $queryArray = [];

            if (!$esistenza_utente) {
                $query_inserimento = 'INSERT INTO utente SET ' . rtrim($query_utente[$v], ', ') . ',' . $gruppo_base . ' valido_da = "' . $data_validita . '"';
                $queryArray[] = ['query' => $query_inserimento, 'message' => 'Errore durante l\'inserimento dell\'utente.'];

                $dettaglio_utente = 'SELECT * FROM utente WHERE id_utente = "' . db_input($id_utente_inserito) . '"';
                $queryArray[] = ['type' => 'select', 'query_select_dettaglio' => $dettaglio_utente, 'message' => 'Errore durante la selezione del dettaglio utente.'];

                $query_inserimento_acm = "INSERT INTO anagrafica_collegati_mandante SET id_mandante ='".$id_mandante."', id_collegato_pratica='".$id_utente_inserito."', id_pratica='".$id_pratica."', ragione_sociale_collegato='".db_input($dettaglio_utente['cognome'].' '.$dettaglio_utente['nome'])."', data_inserimento=NOW()" .((rtrim($query_utente_acm[$v], ', ') != '') ? ','.rtrim($query_utente_acm[$v], ', '):'' );
                $queryArray[] = ['type' => 'insert', 'query_update' => $query_inserimento_acm, 'message' => 'Errore durante l\'inserimento in anagrafica_collegati_mandante.'];
            } else {
                $utenteTrovato = $ris_esistenza_utente->fetch_assoc();
                $id_utente_inserito = $utenteTrovato['id_utente'];

                $dettaglio_utente = 'SELECT * FROM utente WHERE id_utente = "' . db_input($id_utente_inserito) . '"';
                $queryArray[] = ['type' => 'select', 'query_select_dettaglio' => $dettaglio_utente, 'message' => 'Errore durante la selezione del dettaglio utente.'];

                $query_update_acm = "UPDATE anagrafica_collegati_mandante SET id_mandante ='".$id_mandante."' , ragione_sociale_collegato='".db_input($dettaglio_utente['cognome'].' '.$dettaglio_utente['nome'])."', id_pratica='".$id_pratica."', data_inserimento=NOW()" . ((rtrim($query_utente_acm[$v], ', ') != '') ? ','.rtrim($query_utente_acm[$v], ', '):'' )  ." WHERE id_collegato_pratica='".$id_utente_inserito."'";
                $queryArray[] = ['type' => 'update', 'query_update' => $query_update_acm, 'message' => 'Errore durante l\'aggiornamento in anagrafica_collegati_mandante.'];
            }

            $serializedArray = serialize($queryArray);
            // $filename = 'test_emanuele_queries_anag_collegati.php';
            file_put_contents('test_emanuele_queries_anag_collegati.php', $serializedArray);

            // print_r($queryArray);

            // if(!$esistenza_utente) {
            //     // $query_inserimento = 'INSERT INTO utente SET ' . rtrim($query_utente[$v], ', ') . ',' . $gruppo_base . ' valido_da = "' . $data_validita . '"';
            //     $query_inserimento = $mysqli->prepare('INSERT INTO utente SET ' . rtrim($query_utente[$v], ', ') . ',' . $gruppo_base . ' valido_da = "' . $data_validita . '"');

            //     if ($debug) {
            //         print_r($query_inserimento);
            //         echo '<br><br>';
            //     }

            //     // db_query($query_inserimento) or die('ERRORE INSERIMENTO UTENTE: ' . mysql_error());

            //     // $id_utente_inserito = mysql_insert_id();




            //     $dettaglio_utente = $mysqli->prepare('SELECT * FROM utente WHERE id_utente = "' . db_input($id_utente_inserito) . '"');

            //     // $dettaglio_utente = mysql_fetch_array(db_query('SELECT * FROM utente WHERE id_utente = "' . db_input($id_utente_inserito) . '"'));

            //     if ($dettaglio_utente['cognome'] != '' && $dettaglio_utente['partita_iva'] != '') echo '<strong>ID UTENTE: ' . $id_utente_inserito . ' - ' . $dettaglio_utente['cognome'] . ' (' . $dettaglio_utente['partita_iva'] . ')</strong>';
            //     else if ($dettaglio_utente['cognome'] != '' && $dettaglio_utente['codice_fiscale'] != '') echo '<strong>ID UTENTE: ' . $id_utente_inserito . ' - ' . $dettaglio_utente['cognome'] . ' (' . $dettaglio_utente['codice_fiscale'] . ')</strong>';

            //     echo '<br>';


            //     $query_inserimento_acm = $mysqli->prepare("INSERT INTO anagrafica_collegati_mandante SET id_mandante ='".$id_mandante."', id_collegato_pratica='".$id_utente_inserito."', id_pratica='".$id_pratica."', ragione_sociale_collegato='".db_input($dettaglio_utente['cognome'].' '.$dettaglio_utente['nome'])."', data_inserimento=NOW()" .((rtrim($query_utente_acm[$v], ', ') != '') ? ','.rtrim($query_utente_acm[$v], ', '):'' ));
            //     // $query_inserimento_acm="INSERT INTO anagrafica_collegati_mandante SET id_mandante ='".$id_mandante."', id_collegato_pratica='".$id_utente_inserito."', id_pratica='".$id_pratica."', ragione_sociale_collegato='".db_input($dettaglio_utente['cognome'].' '.$dettaglio_utente['nome'])."', data_inserimento=NOW()" .((rtrim($query_utente_acm[$v], ', ') != '') ? ','.rtrim($query_utente_acm[$v], ', '):'' );

            //     db_query($query_inserimento_acm) or die('ERRORE INSERIMENTO UTENTE: ' . mysql_error());
            ...
            //         */
            //     echo '<strong>UTENTE INSERITO</strong> [ID UTENTE: ' . $id_utente_inserito . ']<br>';

            //     if($debug){
            //         echo '<strong>'.$query_inserimento_acm.'</strong><br>';
            //     }
            // }
            // else {
            //     $utenteTrovato = mysql_fetch_assoc($ris_esistenza_utente);
            ...
            //     db_query($query_update_acm) or die('ERRORE INSERIMENTO UTENTE: ' . mysql_error());


            //     echo '<strong>UTENTE ESISTENTE</strong> [ID UTENTE: ' . $utenteTrovato['id_utente'] . ']<br>';
            //     if($debug){
            //         echo '<strong>'.$query_update_acm.'</strong><br>';
            //     }
            // }
        }

        // INSERIMENTO/AGGIORNAMENTO ANAGRAFICA	// FUNZIONA
        if (count($query_anagrafica) > 0) {
            for ($v = 0; $v < count($query_anagrafica); $v++) {
                ...
                if (strpos($query_anagrafica[$v], 'nazione') > 0)
                    db_query('INSERT INTO anagrafica SET ' . rtrim($query_anagrafica[$v], ', ')) or die('ERRORE CREAZIONE ANAGRAFICA: ' . mysql_error());
                else
                    db_query('INSERT INTO anagrafica SET ' . rtrim($query_anagrafica[$v], ', ') . ', nazione_nascita = "IT"') or die('ERRORE CREAZIONE ANAGRAFICA: ' . mysql_error());

                $dettaglio_anagrafica = mysql_fetch_array(db_query('SELECT * FROM anagrafica WHERE id_anagrafica = "' . mysql_insert_id() . '"'));

                echo '<strong>ANAGRAFICA AGGIORNATA</strong> [ID ANAGRAFICA: ' . $dettaglio_anagrafica['id_anagrafica'] . ']<br>';
                db_query('UPDATE utente SET anagrafica = "' . db_input($dettaglio_anagrafica['id_anagrafica']) . '" WHERE id_utente = "' . db_input($id_utente_inserito) . '"') or die('ERRORE AGGIORNAMENTO ANAGRAFICA SU UTENTE: ' . mysql_error());
            }
        }
        ...
        for ($v = 0; $v < count($query_recapito); $v++) {
            if ($query_recapito[$v] != '' && strpos($query_recapito[$v], 'indirizzo = ""') === FALSE && strpos($query_recapito[$v], 'ndirizzo') > 0) {
                ...
                if (($indirizzo_predefinito == 1 || $ind_predefinito == 1) && $v == 0) {
                    if ($debug) echo 'UPDATE recapito SET predefinito = 0 WHERE id_utente = "' . db_input($id_utente_inserito) . '"<br>';
                    db_query('UPDATE recapito SET predefinito = 0 WHERE id_utente = "' . db_input($id_utente_inserito) . '"');
                    //if (isset($id_utente) && $id_utente > 0)
                    //    db_query('UPDATE recapito SET predefinito = 0 WHERE id_utente = "' . db_input($id_utente) . '"');
                }

                if ($debug) print_r('INSERT IGNORE INTO recapito SET id_utente = "' . db_input($id_utente_inserito) . '", ' . rtrim($query_recapito[$v], ', ') . ' ON DUPLICATE KEY UPDATE ' . rtrim($query_recapito[$v], ', '));
                echo '<br><br>';

                $ris = db_query('INSERT IGNORE INTO recapito SET id_utente = "' . db_input($id_utente_inserito) . '", ' . rtrim($query_recapito[$v], ', ') . ' ON DUPLICATE KEY UPDATE ' . rtrim($query_recapito[$v], ', ')) or die('ERRORE AGGIUNTA RECAPITO: ' . mysql_error());
                //if (isset($id_utente) && $id_utente > 0)
                //    $ris2 = db_query('INSERT IGNORE INTO recapito SET id_utente = "' . db_input($id_utente) . '", ' . rtrim($query_recapito[$v], ', ') . ' ON DUPLICATE KEY UPDATE ' . rtrim($query_recapito[$v], ', ')) or die('ERRORE AGGIUNTA RECAPITO: ' . mysql_error());
        ...
        for ($v = 0; $v < count($query_recapito_telefonico); $v++) {
            if ($query_recapito_telefonico[$v] != '' && strpos($query_recapito_telefonico[$v], 'indirizzo = ""') === FALSE && strpos($query_recapito_telefonico[$v], 'tipo_') > 0 && strpos($query_recapito_telefonico[$v], 'ndirizzo') > 0) {
                ...
                if ($recapito_predefinito == 1 || $rec_predefinito == 1) {
                    if ($debug) echo 'UPDATE recapito_telefonico SET principale = 0 WHERE id_utente = "' . db_input($id_utente_inserito) . '"<br>';
                    db_query('UPDATE recapito_telefonico SET principale = 0 WHERE id_utente = "' . db_input($id_utente_inserito) . '"');
                    //if (isset($id_utente) && $id_utente > 0)
                    //    db_query('UPDATE recapito_telefonico SET principale = 0 WHERE id_utente = "' . db_input($id_utente) . '"');
                }

                if ($debug) print_r('INSERT IGNORE INTO recapito_telefonico SET id_utente = "' . db_input($id_utente_inserito) . '", ' . rtrim($query_recapito_telefonico[$v], ', ') . ' ON DUPLICATE KEY UPDATE ' . rtrim($query_recapito_telefonico[$v], ', '));
                echo '<br><br>';

                db_query('INSERT IGNORE INTO recapito_telefonico SET id_utente = "' . db_input($id_utente_inserito) . '", ' . rtrim($query_recapito_telefonico[$v], ', ') . ' ON DUPLICATE KEY UPDATE ' . rtrim($query_recapito_telefonico[$v], ', ')) or die('ERRORE AGGIUNTA RECAPITO TELEFONICO: ' . mysql_error());
                //if (isset($id_utente) && $id_utente > 0)
                //    db_query('INSERT IGNORE INTO recapito_telefonico SET id_utente = "' . db_input($id_utente) . '", ' . rtrim($query_recapito_telefonico[$v], ', ') . ' ON DUPLICATE KEY UPDATE ' . rtrim($query_recapito_telefonico[$v], ', ')) or die('ERRORE AGGIUNTA RECAPITO TELEFONICO: ' . mysql_error());
        ...
        // INSERIMENTO COLLEGATO ANAGRAFICA // FUNZIONA
        if (isset($id_utente) && $id_utente > 0) {
            $query_aggiornamento_legame = 'INSERT INTO collegati
												SET id_utente = "' . $id_utente_inserito . '",
													id_tipo = "' . $id_tipologia_collegato_anagrafica . '",
													id_collegato = "' . $id_utente . '"';
            db_query($query_aggiornamento_legame);// or rollbackOperations(mysql_error('ERRORE COLLEGAMENTO ALTRO COLLEGATO UTENTE: '.mysql_error()));

            //$array_rollback_acquisizione[] = 'DELETE FROM collegati WHERE id = "'.db_input(mysql_insert_id()).'"';

            $query_aggiornamento_legame = 'INSERT INTO collegati
												SET id_utente = "' . $id_utente . '",
													id_tipo = "' . $id_tipologia_collegato_anagrafica . '",
													id_collegato = "' . $id_utente_inserito . '"';
            db_query($query_aggiornamento_legame);// or rollbackOperations(mysql_error('ERRORE COLLEGAMENTO ALTRO COLLEGATO UTENTE: '.mysql_error()));
        ...
        // INSERIMENTO COLLEGATO PRATICA
        if (isset($id_pratica) && $id_pratica > 0) {


            foreach ($dett_univoco as $key => $val) {

                if(isset($val['array_prt']) && is_array($val['array_prt'])){

                    foreach ($val['array_prt'] as $id_prt) {

                        $query_inserimento_collegati_pratica = 'INSERT INTO collegati_pratica 
															SET id_pratica = "' . db_input($id_prt) . '",
																id_tipo = "' . $id_tipologia_collegato_pratica . '",
																id_collegato = "' . db_input($id_utente_inserito) . '"';
                        db_query($query_inserimento_collegati_pratica);// or rollbackOperations('ERRORE INSERIMENTO COLLEGATO PRATICA (ALTRO): '.mysql_error());
                    ...
                    $query_inserimento_collegati_pratica = 'INSERT INTO collegati_pratica 
															SET id_pratica = "' . db_input($val['id_pratica']) . '",
																id_tipo = "' . $id_tipologia_collegato_pratica . '",
																id_collegato = "' . db_input($id_utente_inserito) . '"';
                    db_query($query_inserimento_collegati_pratica);// or rollbackOperations('ERRORE INSERIMENTO COLLEGATO PRATICA (ALTRO): '.mysql_error());
        ...
        <div class="page-content">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="#">Acquisizione Collegati</a>
                    </li>
                </ul>
            </div>
            <!-- END PAGE HEADER-->
                ...
                <div class="col-md-12">
                    <div class="tabbable tabbable-custom">
                        <ul class="nav nav-tabs">
                            <li class="active">
                                <a href="#tab_0" data-toggle="tab">Acquisizione Collegati</a>
                            </li>
                        </ul>
                                ...
                                <form name="formElementi" id="form_sample_22" method="POST"
                                    ...
                                    <div class="form-body">
                                        ...
                                        if (isset($_POST['test']) && $_POST['test'] != '') {
                                            ...
                                            $tipi_collegati_anagrafica = $_POST['linked_user_type'];
                                            $tipi_collegati_pratica = $_POST['linked_type'];
                                                    ...
                                                    <div class="portlet-body">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-md-12">
                                                                ...
                                                                $ind_predefinito = (isset($_POST['ind_predefinito']) && $_POST['ind_predefinito'] != '' && $_POST['ind_predefinito'] == '1') ? '1' : '0';
                                                                $ind_corrispondenza = (isset($_POST['ind_corrispondenza']) && $_POST['ind_corrispondenza'] != '' && $_POST['ind_corrispondenza'] == '1') ? '1' : '0';
                                                                $ind_corrispondenza_predefinito = (isset($_POST['ind_corrispondenza_predefinito']) && $_POST['ind_corrispondenza_predefinito'] != '' && $_POST['ind_corrispondenza_predefinito'] == '1') ? '1' : '0';

                                                                $ind_attivo = (isset($_POST['ind_primario_attivo']) && $_POST['ind_primario_attivo'] != '' && $_POST['ind_primario_attivo'] == '1') ? '1' : '0';

                                                                $ind_tipo = (isset($_POST['tipo_indirizzo_predefinito']) && $_POST['tipo_indirizzo_predefinito'] != '' && $_POST['tipo_indirizzo_predefinito'] == '1') ? $_POST['tipo_indirizzo_predefinito'] : '0';

                                                                $rec_predefinito = (isset($_POST['rec_predefinito']) && $_POST['rec_predefinito'] != '' && $_POST['rec_predefinito'] == '1') ? '1' : '0';
                                                                $rec_corrispondenza = (isset($_POST['rec_invio_predefinito']) && $_POST['rec_invio_predefinito'] != '' && $_POST['rec_invio_predefinito'] == '1') ? '1' : '0';
                                                                $rec_attivo = (isset($_POST['rec_primario_attivo']) && $_POST['rec_primario_attivo'] != '' && $_POST['rec_primario_attivo'] == '1') ? '1' : '0';
                                                                ...
                                                                $mysqli->autocommit(FALSE);
                                                                try{
                                                                    $update_acquisizione_pending = $mysqli->prepare("UPDATE `ed_acquisizioni_file` SET stato = 'Pending' WHERE id = '$id_acquisizione' AND tipologia = 'ANAG.COLLEGATI'");
                                                                ...
                                                                if (count($eventi_strutturati) > 0) {
                                                                    ...
                                                                    foreach ($eventi_strutturati as $key => $value) {
                                                                        ...
                                                                        echo "executeEvent( '$key', '" . rtrim($value, ',') . "' );";
                                                                        echo "$('#loader1').hide();";
                                                                        echo '</script>';
                                                                    }
                                            ...
                                            if (isset($_GET['config']) && $_GET['config'] != '') {
                                                $query_recupero_configurazioni = 'SELECT * FROM template_acquisizione_collegati WHERE id = "' . $_GET['config'] . '"';
                                                ...
                                                $configurazione_tipologie_collegato_anagrafica = explode(';', $configurazione['tipologie_collegato_anagrafica']);
                                                $configurazione_tipologie_collegato_pratica = explode(';', $configurazione['tipologie_collegato_pratica']);
                                                        ...
                                                        <div class="col-md-6" style="line-height:33px;">
                                                            ...
                                                            <div>
                                                                <select id="inserimento_configurazione"
                                                                    ...
                                                                    if (isset($_GET['config']) && $_GET['config'] != '') {
                                                                        $query = 'SELECT *
                                                                                                FROM template_acquisizione_collegati
                                                        ...
                                                        <div class="col-md-3">
                                                            ...
                                                            <div style="margin-bottom: 7px;">
                                                                <input type="checkbox"
                                                                       ...
                                                                       data-off-text="&nbsp;<i class='fa fa-circle-o'></i>&nbsp;" <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione['indirizzo_predefinito'] == 1) echo 'checked'; ?>><input
                                                        ...
                                                        <div class="col-md-3">
                                                            ...
                                                            <div style="margin-bottom: 7px;">
                                                                <input type="checkbox"
                                                                       ...
                                                                       data-on-text="&nbsp;<i class='fa fa-check-circle-o'></i>&nbsp;"
                                                                       data-off-text="&nbsp;<i class='fa fa-circle-o'></i>&nbsp;" <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione['indirizzo_corrispondenza'] == 1) echo 'checked'; ?>><input
                                                                        id="checkHiddenCorrisp" type="hidden" value="0"
                                                        ...
                                                        <div class="col-md-3">
                                                            ...
                                                            <div style="margin-bottom: 7px;">
                                                                <input type="checkbox"
                                                                       ...
                                                                       name="ind_primario_attivo" value="1"
                                                                       data-on-color="success" data-off-color="warning"
                                                                       data-on-text="&nbsp;<i class='fa fa-check-circle-o'></i>&nbsp;"
                                                                       data-off-text="&nbsp;<i class='fa fa-circle-o'></i>&nbsp;" <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione['indirizzo_attivo'] == 1) echo 'checked'; ?>><input
                                                                        id="checkHiddenIndAct" type="hidden" value="0"
                                                        ...
                                                        <div class="col-md-3">
                                                            ...
                                                            <div style="margin-bottom: 7px;">
                                                                <input type="checkbox"
                                                                       ...
                                                                       data-off-color="warning"
                                                                       data-on-text="&nbsp;<i class='fa fa-check-circle-o'></i>&nbsp;"
                                                                       data-off-text="&nbsp;<i class='fa fa-circle-o'></i>&nbsp;" <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione['recapito_predefinito'] == 1) echo 'checked'; ?>><input
                                                                        id="checkHiddenRecPred" type="hidden" value="0"
                                                        ...
                                                        <div class="col-md-3">
                                                            ...
                                                            <div style="margin-bottom: 7px;">
                                                                <input type="checkbox"
                                                                       ...
                                                                       data-on-color="success" data-off-color="warning"
                                                                       data-on-text="&nbsp;<i class='fa fa-check-circle-o'></i>&nbsp;"
                                                                       data-off-text="&nbsp;<i class='fa fa-circle-o'></i>&nbsp;" <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione['recapito_invio'] == 1) echo 'checked'; ?>><input
                                                                        id="checkHiddenRecCorrisp" type="hidden"
                                                        ...
                                                        <div class="col-md-3">
                                                            ...
                                                            <div style="margin-bottom: 7px;">
                                                                <input type="checkbox"
                                                                       ...
                                                                       data-on-color="success" data-off-color="warning"
                                                                       data-on-text="&nbsp;<i class='fa fa-check-circle-o'></i>&nbsp;"
                                                                       data-off-text="&nbsp;<i class='fa fa-circle-o'></i>&nbsp;" <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione['recapito_attivo'] == 1) echo 'checked'; ?>><input
                                                                        id="checkHiddenRecAct" type="hidden" value="0"
                                                            ...
                                                            <span class="line"
                                                                  ...
                                                                  data-line="<?php echo $i ?>">
                                                                            ...
                                                                            <div class="col-md-4">
                                                                                <div>
                                                                                    <select name="campo[]"
                                                                                        ...
                                                                                        <optgroup
                                                                                                label="Tipologie Collegati">
                                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'altro*-*tipologia_collegato_pratica') echo 'selected'; ?> value="altro*-*tipologia_collegato_pratica">COLLEGATO PRATICA - Tipologia Collegato</option>
                                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'altro*-*tipologia_collegato_anagrafica') echo 'selected'; ?> value="altro*-*tipologia_collegato_anagrafica">COLLEGATO ANAGRAFICA - Tipologia Collegato</option>
                                                                                        </optgroup>
                                                                                        <optgroup
                                                                                                label="Dati ANAGRAFICA">
																							<option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'anagrafica*-*nome') echo 'selected'; ?> value="anagrafica*-*nome">ANAGRAFICA - Nome</option>
																							<option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'anagrafica*-*cognome') echo 'selected'; ?> value="anagrafica*-*cognome">ANAGRAFICA - Cognome</option>
                                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'anagrafica*-*codice_anagrafico_mandante') echo 'selected'; ?> value="anagrafica*-*codice_anagrafico_mandante">ANAGRAFICA - Codice Anagrafico Mandante</option>
                                                                            ...
                                                                            <div class="col-md-4" style="height: 35px;">
                                                                                ...
                                                                                $mostra_tipologie_collegato_anagrafica = false;
                                                                                $mostra_tipologie_collegato_pratica = false;
                                                                                ...
                                                                                if (isset($_GET['config']) && $_GET['config'] != '') {
                                                                                    ...
                                                                                    if ($configurazione_campi[$i] == 'altro*-*id_anagrafica' /*|| $configurazione_campi[$i] == 'anagrafica*-*recapito'*/) {
                                                                                        $mostra_tipologie_collegato_anagrafica = true;
                                                                                    }
                                                                                    if ($configurazione_campi[$i] == 'altro*-*id_pratica' || $configurazione_campi[$i] == 'altro*-*codice_fiscale' || $configurazione_campi[$i] == 'altro*-*riferimento_mandante_1' || $configurazione_campi[$i] == 'altro*-*riferimento_mandante_2' || $configurazione_campi[$i] == 'altro*-*codice_anagrafico_mandante' || $configurazione_campi[$i] == 'altro*-*id_mandante'   /*|| $configurazione_campi[$i] == 'anagrafica*-*recapito'*/) {
                                                                                        $mostra_tipologie_collegato_pratica = true;
                                                                                    }
                                                                                }
                                                                                ...
                                                                                <div style="float: left; width: 230px; margin-right: 10px;"
                                                                                     class="extra-field-<?php echo $i ?> field_user_linked_type-<?php echo $i ?> <?php if (!$mostra_tipologie_collegato_anagrafica) echo 'hidden'; ?>">
                                                                                    <select name="linked_user_type[]"
                                                                                            class="form-control <?php if ($i < $line) echo 'select2me'; ?> tipologie_collegato_anagrafica">
                                                                                    	<option></option>
                                                                                        <?php
                                                                                        ...
                                                                                        foreach ($tipologie as $tipologia) {
                                                                                            ?>
                                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_tipologie_collegato_anagrafica[$i] == $tipologia['id']) echo 'selected'; ?> value="<?php echo $tipologia['id']; ?>"><?php echo $tipologia['descrizione']; ?></option>
                                                                                            <?php
                                                                                        }
                                                                                        ?>
                                                                                    </select>
                                                                                ...
                                                                                <div style="float: left; width: 230px; margin-right: 10px;"
                                                                                     class="extra-field-<?php echo $i ?> field_linked_type-<?php echo $i ?> <?php if (!$mostra_tipologie_collegato_pratica) echo 'hidden'; ?>">
                                                                                    <select name="linked_type[]"
                                                                                            class="form-control <?php if ($i < $line) echo 'select2me'; ?> tipologie_collegato_pratica">
                                                                                    	<option></option>
                                                                                        <?php
                                                                                        $tipologie = recuperaTipologieCollegato();
                                                                                        ...
                                                                                        foreach ($tipologie as $tipologia) {
                                                                                            ?>
                                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_tipologie_collegato_pratica[$i] == $tipologia['id']) echo 'selected'; ?> value="<?php echo $tipologia['id']; ?>"><?php echo $tipologia['descrizione']; ?></option>
                                                                                            <?php
                                                                                        }
                                                                                        ?>
                                                                                    </select>
                                                            ...
                                                            <div class="col-md-4">
                                                                <div>
                                                                    <select name="campo[]"
                                                                        ...
                                                                        <optgroup label="Tipologie Collegati">
                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'altro*-*tipologia_collegato_pratica') echo 'selected'; ?>
                                                                                    value="altro*-*tipologia_collegato_pratica">
                                                                                COLLEGATO PRATICA - Tipologia Collegato
                                                                            </option>
                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'altro*-*tipologia_collegato_anagrafica') echo 'selected'; ?>
                                                                                    value="altro*-*tipologia_collegato_anagrafica">
                                                                                COLLEGATO ANAGRAFICA - Tipologia
                                                                                Collegato
                                                                            </option>
                                                                        </optgroup>
                                                                        <optgroup label="Dati ANAGRAFICA">
                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'anagrafica*-*nome') echo 'selected'; ?>
                                                                                    value="anagrafica*-*nome">ANAGRAFICA
                                                                                - Nome
                                                                            </option>
                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'anagrafica*-*cognome') echo 'selected'; ?>
                                                                                    value="anagrafica*-*cognome">
                                                                                ANAGRAFICA - Cognome
                                                                            </option>
                                                                            <option <?php if (isset($_GET['config']) && $_GET['config'] != '' && $configurazione_campi[$i] == 'anagrafica*-*codice_anagrafico_mandante') echo 'selected'; ?>
                                                                                    value="anagrafica*-*codice_anagrafico_mandante">
                                                                                ANAGRAFICA - Codice Anagrafico Mandante
                                                                            </option>
                                                                    ...
                                                                    <input class="form-control posizioni"
                                                                           value="<?php if (isset($_GET['config']) && $_GET['config'] != '') echo $configurazione_posizioni[$i]; ?>"
                                                                           type="hidden" name="posizione[]">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-2">
                                                                <div>
...
<script>
    ...
    function displayOptionalFields(elem, index) {
        ...
        } else if (elem.val() == 'altro*-*id_pratica' || elem.val() == 'altro*-*codice_fiscale' || elem.val() == 'altro*-*riferimento_mandante_1' || elem.val() == 'altro*-*riferimento_mandante_2' || elem.val() == 'altro*-*codice_anagrafico_mandante' || elem.val() == 'altro*-*id_mandante') {
            $('.extra-field-' + index).addClass('hidden');
            $('.field_linked_type-' + index).removeClass('hidden');
        } else if (elem.val() == 'altro*-*id_anagrafica') {
            $('.extra-field-' + index).addClass('hidden');
            $('.field_user_linked_type-' + index).removeClass('hidden');
        } else {
            $('.extra-field-' + index).addClass('hidden');
        }
    ...
    function salva_configurazione(desc) {
        ...
        var input_tipologie_collegato_pratica = $('.tipologie_collegato_pratica').serialize();
        var input_tipologie_collegato_anagrafica = $('.tipologie_collegato_anagrafica').serialize();
        ...
        $.ajax({
            ...
            type: "POST",
            data: {
                action: 'salva-configurazione-acquisizione-collegati',
                ...
                tipologie_collegato_pratica: input_tipologie_collegato_pratica,
                tipologie_collegato_anagrafica: input_tipologie_collegato_anagrafica,
    ...
    function modifica_configurazione(identity) {
        ...
        $.ajax({
            ...
            type: "POST",
            data: {
                action: 'modifica-configurazione-acquisizione-collegati',
                id: identity,
                id_mandante: input_mandante,
                tipo_file: input_tipo_file,
                riga: input_riga,
    ...
    function elimina_configurazione(identity) {
        if (confirm('Siete sicuri di voler eliminare la configurazione selezionata?')) {
            $.ajax({
                ...
                type: "POST",
                data: {
                    action: 'elimina-configurazione-acquisizione-collegati',
                    id: identity
                },
                success: function (result) {
                    alert(result);
    ...
    function compila_configurazioni(id) {
        $.ajax({
            url: "form_actions.php",
            type: "POST",
            data: {action: 'ricerca-configurazioni-da-mandante-collegati', id_mandante: id},
            success: function (result) {
                //console.log(result);
